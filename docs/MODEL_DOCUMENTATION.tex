...
\title{\textbf{Multimodal Financial Direction Forecasting Using Deep Learning:\\Model Development \& Experimental Results\\(Phases 3--11 Documentation)}}
\author{
    \textbf{Rishi Raval}\\
...
\begin{abstract}
This document presents the complete model development pipeline for a multimodal stock direction forecasting system targeting 10 major technology stocks over a 10-year period (2016--2026). Starting from preprocessing (Phase~3), it covers training of individual CNN-BiLSTM price, FinBERT document, and Graph Attention Network (GAT) encoders (Phases~4--5), their integration into a gated multi-task fusion model (Phase~6), systematic ablation studies (Phase~7), and four targeted V1 improvements including confidence-filtered trading analysis. A comprehensive V2 redesign (Phase~8--10) addressed critical data-leakage issues, replaced the 5-day binary cross-entropy objective with a 60-day ListNet ranking loss, introduced a new macroeconomic modality (VIX, yields, credit spreads), and adopted AMP fp16 training. The V2 pipeline achieves a direction AUC of \textbf{0.5989} and gate weights dominated by GAT (41.2\%), representing a $+16\%$ improvement over the V1 baseline. Strategic upgrades (Phase~9) evaluated regime-switching ensembles, realistic backtesting, model optimisation, and enhanced cross-stock features, finding the clean V2 fusion to remain the strongest model. Phase~11 applied three targeted improvements: macro encoder expansion to 128-d (V2.1, AUC~0.5917, macro gate $12.6\% \to 31.7\%$), confidence-filtered prediction (V2.1 best AUC~\textbf{0.6127} at 32\% coverage), and a weekly long-short backtest showing \textbf{$+10.4\%$~total return} and Sharpe~0.18 at zero transaction cost~(break-even~$\approx$10\,bp).
\end{abstract}
...
\begin{itemize}[noitemsep,topsep=0pt,leftmargin=*]
    ...
    \item \textbf{V1 Model Improvements:} Loss tuning, dynamic graphs, longer horizons, ensemble (Section~\ref{sec:improvements})
    \item \textbf{V1 Confidence-Filtered Trading:} Selective prediction via confidence thresholds (Section~\ref{sec:confidence})
    \item \textbf{Phase 8:} Comprehensive V2 Improvements --- leakage fix, GradNorm, walk-forward, ranking loss (Section~\ref{sec:v2improvements})
    \item \textbf{Phase 9:} Strategic Model Upgrades --- regime-switching, backtesting, optimisation, cross-stock features
    \item \textbf{Phase 10:} V2 Pipeline Redesign --- 4-modality, ListNet loss, macro encoder, AMP fp16, 60-day horizon
    \item \textbf{Phase 11:} Targeted Improvements --- V2.1 macro expansion (128-d), confidence filtering, weekly backtest
\end{itemize}
...
% ======================================================================
\newpage
\section{Phase 9: Strategic Model Upgrades}

Phase 9 investigates four strategic improvement areas beyond the core model architecture:
regime-switching ensembles, realistic backtesting, model optimization for deployment,
and enhanced cross-stock feature engineering.

\subsection{Phase 9A: Regime-Switching Ensemble}

\subsubsection{Motivation}
Market behavior varies across regimes (bull, bear, sideways). A regime-switching
ensemble trains specialist expert models for each market state, potentially capturing
regime-specific patterns that a single model averages over.

\subsubsection{Method}
\begin{enumerate}
    \item \textbf{Regime Detection}: KMeans clustering (3 states) on daily cross-stock 
    market features: 5-day rolling mean return and rolling dispersion, computed from 
    \textit{historical} daily log returns (backward-looking).
    \item \textbf{Expert Training}: One Clean Fusion model per regime, trained only on 
    samples assigned to that regime.
    \item \textbf{Routing}: At test time, each sample is routed to its regime-specific 
...
\begin{table}[H]
\centering
\caption{Phase 9A: Regime-Switching Expert Results}
\begin{tabular}{lrrrrr}
\toprule
\textbf{Regime} & \textbf{Days} & \textbf{Train N} & \textbf{Val AUC} & \textbf{Test AUC} & \textbf{Test N} \\
\midrule
Bear     & 752 & 8,560  & 0.6023 & 0.5060 & 5,120 \\
Sideways & 921 & 13,370 & 0.5641 & 0.4864 & 2,920 \\
Bull     & 855 & 12,520 & 0.5688 & 0.5060 & 2,580 \\
\midrule
\textbf{Combined} & \textbf{2,528} & --- & --- & \textbf{0.4976} & \textbf{10,620} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Finding}: The regime ensemble (AUC=0.498) \textit{underperforms} the unified
model (AUC=0.531). Splitting data by regime reduces per-expert training samples, increasing
variance without improving bias. The regime detection itself is noisy with only 10 stocks.

\subsection{Phase 9B: Backtesting Engine with Transaction Costs}

\subsubsection{Motivation}
A model's statistical AUC is insufficient to assess real-world trading viability. We need
to measure performance after accounting for transaction costs, slippage, and portfolio
construction choices.

\subsubsection{Strategies Tested}
\begin{itemize}
    \item \textbf{Equal Weight}: Buy top-$K$ predicted stocks ($K \in \{3, 5\}$) with equal allocation
    \item \textbf{Confidence-Weighted}: Position size proportional to model probability
    \item \textbf{Risk-Parity}: Inverse-volatility weighted positions
    \item \textbf{Buy-and-Hold}: Equal-weight all 10 stocks (benchmark)
\end{itemize}

Fee levels tested: 0bp, 5bp, and 10bp per trade.

\subsubsection{Results}

\begin{table}[H]
\centering
\caption{Phase 9B: Backtesting Results (2022--2025 Test Period)}
\begin{tabular}{lrrrr}
\toprule
\textbf{Strategy} & \textbf{Net Return} & \textbf{Sharpe} & \textbf{Max DD} & \textbf{Win Rate} \\
\midrule
Buy-and-Hold           & +1,981\% & 1.23  & $-$73.0\% & 60.8\% \\
EW Top-5 (0bp)         & +1,263\% & 1.66  & $-$37.5\% & 61.6\% \\
EW Top-3 (0bp)         & +1,404\% & 1.36  & $-$45.9\% & 58.4\% \\
\midrule
EW Top-5 (5bp)         & $-$93.3\% & $-$1.54 & $-$93.8\% & 37.7\% \\
EW Top-3 (5bp)         & $-$37.5\% & $-$0.11 & $-$68.8\% & 48.2\% \\
Confidence $> 0.52$    & $-$100.0\% & $-$4.33 & $-$100.0\% & 20.5\% \\
Risk-Parity Top-5      & $-$94.2\% & $-$1.76 & $-$94.5\% & 36.2\% \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Critical Finding}: With even modest transaction costs (5bp), all model-based
strategies become deeply unprofitable. At AUC=0.53, the direction signal is too weak to
overcome daily rebalancing costs. The zero-fee ``alpha'' is entirely consumed by transaction
friction. Profitable deployment would require AUC $\geq$ 0.55 consistently or longer holding
periods to reduce turnover.

\subsection{Phase 9C: Model Optimization (FP16 / INT8 / ONNX)}

\subsubsection{Results}

\begin{table}[H]
\centering
\caption{Phase 9C: Inference Optimization Benchmarks (10,620 samples)}
\begin{tabular}{lrrrr}
\toprule
\textbf{Format} & \textbf{Time (ms)} & \textbf{Size (MB)} & \textbf{Speedup} & \textbf{AUC} \\
\midrule
FP32 (GPU) & 570 & 2.18 & 1.00$\times$ & 0.531 \\
FP16 (GPU) & 637 & 1.09 & 0.89$\times$ & --- \\
INT8 (CPU) & 679 & $\sim$1.1 & 0.84$\times$ & 0.525 \\
ONNX       & \multicolumn{4}{c}{Export failed (missing onnxscript)} \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Finding}: FP16 and INT8 show no speedup because the model is very small 
(545K parameters, 2.2 MB). Quantization overhead exceeds compression savings at this scale. 
Benefits would appear at $>$10M parameters. INT8 preserves prediction quality (AUC=0.525).

\subsection{Phase 9D: Enhanced Cross-Stock Features}

\subsubsection{Method}
Five cross-stock features computed from \textbf{historical} (backward-looking) data only:
\begin{enumerate}
    \item \textbf{Sector Past Momentum}: Mean rolling 5-day past return of sector peers
    \item \textbf{Market Past Momentum}: Mean rolling 5-day past return of all 10 stocks
    \item \textbf{Cross-Stock Dispersion}: Std dev of past returns across all stocks
    \item \textbf{Volatility Regime}: Realized vol relative to cross-sectional median
    \item \textbf{Sector Rank}: Percentile rank within sector by past 5-day return
\end{enumerate}

These are fused via a 6-head attention gate (5 original modalities + enhanced features).

\subsubsection{Data Leakage Discovery \& Fix}

The initial implementation accidentally used \texttt{direction\_5d\_return}
(forward-looking 5-day return) from peer stocks to compute sector/market momentum.
Since tech stocks are highly correlated, peer forward returns leaked the target---producing 
a spurious AUC=0.939 and gate weight of 62.5\% on the enhanced features.

\textbf{Fix}: Replaced with \texttt{log\_return} (historical daily close-to-close return) 
and computed rolling 5-day \textit{past} sums. The corrected AUC dropped to a 
realistic 0.5226.
...
\begin{table}[H]
\centering
\caption{Phase 9D: Enhanced Features --- Leaky vs Fixed}
\begin{tabular}{lrrl}
\toprule
\textbf{Model} & \textbf{Test AUC} & \textbf{Enhanced Gate Wt.} & \textbf{Status} \\
\midrule
Baseline (Clean Fusion)   & 0.5312 & --- & Valid \\
Enhanced (leaky features) & 0.9390 & 62.5\% & \textcolor{red}{INVALID} \\
\textbf{Enhanced (fixed)} & \textbf{0.5226} & \textbf{22.5\%} & \textbf{Valid} \\
\bottomrule
\end{tabular}
\end{table}

\noindent Gate weight distribution (fixed model): Price 34.2\%, GAT 24.3\%, Document 10.8\%, 
Surprise 8.2\%, Enhanced 22.5\%. The model correctly assigns non-trivial weight to cross-stock 
features, but they slightly \textit{hurt} test performance (0.523 vs 0.531), indicating these 
features add noise rather than signal.

\subsection{Phase 9 Summary}

\begin{table}[H]
\centering
\caption{Phase 9 Summary of All Strategic Upgrades}
\begin{tabular}{llrl}
\toprule
\textbf{Phase} & \textbf{Method} & \textbf{Test AUC} & \textbf{Verdict} \\
\midrule
9A & Regime-switching ensemble      & 0.4976 & Negative (data splitting hurts) \\
9B & Backtesting (5bp fees)         & ---    & All strategies unprofitable \\
9C & FP16/INT8 optimization         & 0.525  & No speedup (model too small) \\
9D & Enhanced cross-stock features  & 0.5226 & Marginal negative vs baseline \\
\midrule
\multicolumn{2}{l}{\textbf{Best model: Phase 8 V2 Ranking Loss}} & \textbf{0.5312} & \textbf{Unchanged} \\
\bottomrule
\end{tabular}
\end{table}

\noindent None of the Phase 9 strategic upgrades improve upon the Phase 8 V2 ranking loss 
model (AUC=0.5312). The key lessons:
\begin{enumerate}
    \item \textbf{Regime splitting hurts} when individual regimes have insufficient data
    \item \textbf{Transaction costs dominate} at current signal strength (AUC=0.53)
    \item \textbf{Model compression} is irrelevant at 545K parameter scale
    \item \textbf{Cross-stock features from public data} add noise, not alpha
    \item \textbf{Data leakage vigilance} is essential---forward-looking peer returns inflated AUC from 0.52 to 0.94
\end{enumerate}


% ===========================================================================
\section{Phase 10: V2 Pipeline --- 4-Modality Redesign with ListNet and Macro}
% ===========================================================================

\subsection{Motivation}

After Phase 9, a systematic code review identified three critical issues in the V1 pipeline:
\begin{enumerate}
    \item \textbf{Surprise data leakage (critical)}: The 3-dimensional surprise feature vector
          $[\texttt{is\_beat}, \texttt{is\_miss}, \texttt{recency}]$ directly encodes the
          BEAT/MISS earnings label as input, causing the surprise gate weight to inflate to 39.2\%
          and the surprise task head AUC to reach a perfect 1.000. All V1 results involving the
          surprise modality are invalidated.
    \item \textbf{News modality ineffectiveness}: The news FinBERT+BiGRU modality covers only
          2.1\% of trading days with only 0.021 mean gate weight. The model learns to ignore it.
    \item \textbf{GPU underutilization}: No AMP, batch size 32, cuDNN benchmark off---the RTX 2050
          VRAM (4.29 GB) was barely utilized during training.
\end{enumerate}

\subsection{V2 Architectural Changes}

\subsubsection{Removed Components}
\begin{itemize}
    \item \textbf{News modality} (\texttt{news\_model.py}, \texttt{news\_dataset.py}, \texttt{train\_news.py}):
          removed due to 2.1\% coverage and negligible contribution.
    \item \textbf{Surprise MLP modality} and \textbf{Surprise task head}: removed due to leakage.
    \item \textbf{Dynamic graph} (\texttt{dynamic\_graph.py}): showed $<+0.001$ AUC improvement.
    \item \textbf{Regime-switching ensemble}: negative result (AUC 0.498 vs 0.531 baseline).
\end{itemize}

\subsubsection{Added Components}
\begin{itemize}
    \item \textbf{Macro modality (NEW)}: 12-dimensional backward-looking market state vector
          encoding VIX, Treasury yields, SPY momentum, yield curve spread, credit spreads,
          gold momentum, and market breadth. Computed daily from freely available Yahoo Finance
          data (\^{V}IX, TLT, IEF, SPY, QQQ, SHY, GLD, HYG, LQD), lagged 1 day (no leakage).
    \item \textbf{ListNet ranking loss}: replaces binary cross-entropy for direction prediction.
          Groups samples by trading date and optimizes cross-stock ranking:
          $$L_{\text{ListNet}} = \frac{1}{|D|}\sum_{d \in D} \text{KL}\!\left(\text{softmax}(y_d) \,\|\, \text{softmax}(\hat{y}_d)\right)$$
          This removes common market beta noise that inflates difficulty.
    \item \textbf{60-day primary horizon}: moved from 5-day to 60-day as primary target,
          motivated by Phase 8 finding that 60-day improved AUC from 0.524 to 0.569.
    \item \textbf{AMP fp16 training}: \texttt{torch.amp.autocast} + \texttt{GradScaler}
          wired into all training loops. Batch size 1024 (fusion), 512 (price), 256 (GAT).
          cuDNN benchmark and TF32 enabled.
    \item \textbf{Fixed surprise features}: 5-dimensional backward-looking features
          $[\text{surprise\_pct}, \text{surprise\_mag\_norm}, \text{recency\_decay},
          \text{trailing\_3q\_beat\_rate}, \text{eps\_revision\_dir}]$ used as gating
          conditioning signal only (not as a modality, not as a prediction target).
\end{itemize}

\subsubsection{V2 Fusion Architecture}

The V2 \texttt{MultimodalFusionModel} fuses 4 modalities via sigmoid gating:

\begin{table}[H]
\centering
\caption{V2 Modality Dimensions}
\begin{tabular}{lll}
\toprule
\textbf{Modality} & \textbf{Dimension} & \textbf{Encoder} \\
\midrule
Price    & 256-d & CNN-BiLSTM (60-day OHLCV windows) \\
GAT      & 256-d & Graph Attention Network (10-node static) \\
Document & 768-d & FinBERT attention-pooled (10-K chunks) \\
Macro    & 32-d  & MLP-3L (12 macro features $\to$ 32-d) -- NEW \\
\midrule
Gating input & 5-d & Surprise features (conditioning only, not modality) \\
\bottomrule
\end{tabular}
\end{table}

Gating input: $[p_\text{price}, p_\text{gat}, p_\text{doc}, p_\text{macro}, \text{surprise\_feat}] \in \mathbb{R}^{512+5}$,
processed by \texttt{Linear(517, 256) $\to$ GELU $\to$ Linear(256, 4) $\to$ Sigmoid}.

Combined loss: $L = 0.7 \times L_{\text{ListNet}}(\text{direction}) + 0.3 \times L_{\text{MSE}}(\text{volatility})$.
Total parameters: 550,919 (vs 545,443 V1).

\subsection{V2 Individual Model Results}

\begin{table}[H]
\centering
\caption{V2 Phase 4--5 Individual Model Results (60-day direction, test set)}
\begin{tabular}{llrrrl}
\toprule
\textbf{Phase} & \textbf{Model} & \textbf{AUC} & \textbf{Acc} & \textbf{F1} & \textbf{Notes} \\
\midrule
4A & Price CNN-BiLSTM & 0.5504 & 0.5784 & 0.6867 & 60d, AMP, early stop ep.6 \\
4B & Document FinBERT  & 0.4706 & 0.8500 & ---    & 82 filings; small dataset \\
4C & Macro MLP-3L      & 0.5031 & 0.6254 & ---    & New modality; ep.19 \\
5  & GAT (static)      & 0.5777 & 0.5777 & 0.6669 & 60d, AMP, early stop ep.10 \\
5  & No-GAT ablation   & 0.5515 & 0.5637 & 0.6586 & Encoder + head only \\
\midrule
\multicolumn{2}{l}{Majority baseline (60d UP/DOWN)} & ---   & 0.6254 & --- & Heavy UP bias \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{Note on Document model}: With only 82 filing samples split 52/10/20
(train/val/test), the document model overfits. Val accuracy reaches 1.000 by epoch 3;
AUC of 0.471 on the 20-sample test set is not a reliable estimate.

\noindent \textbf{GAT improvement}: +2.6\% AUC over No-GAT (0.5777 vs 0.5515),
consistent with V1 (+2.3\%). Graph attention over the tech-stock covariance structure
provides reproducible signal.

\subsection{V2 Fusion Model Results}

\begin{table}[H]
\centering
\caption{V2 Fusion Model Results (60-day direction, test set)}
\begin{tabular}{lrl}
\toprule
\textbf{Metric} & \textbf{Value} & \textbf{Notes} \\
\midrule
Direction AUC          & \textbf{0.5989} & Primary metric \\
Direction Accuracy     & 0.5920 & \\
Direction F1           & 0.6732 & \\
Majority baseline acc. & 0.6254 & Heavy UP bias in 60d labels \\
Volatility RMSE        & 0.1686 & \\
Volatility R$^2$       & 0.3350 & \\
Training epochs        & 11     & Early stop (patience=5) \\
\bottomrule
\end{tabular}
\end{table}

\subsection{V2 Fusion Gate Weights}

\begin{table}[H]
\centering
\caption{V2 Fusion Gate Weights (test set mean)}
\begin{tabular}{lrr}
\toprule
\textbf{Modality} & \textbf{V2 Gate Weight} & \textbf{V1 Gate Weight (for reference)} \\
\midrule
GAT (graph structure) & \textbf{0.4118} (41.2\%) & 0.1891 (18.9\%) \\
Price (CNN-BiLSTM)    & 0.2357 (23.6\%) & 0.2140 (21.4\%) \\
Document (FinBERT)    & 0.2269 (22.7\%) & 0.2440 (24.4\%) \\
Macro (MLP-3L) -- NEW & 0.1255 (12.6\%) & --- \\
\textit{~~News (removed)}      & \textit{---}    & \textit{0.0210 (2.1\%)} \\
\textit{~~Surprise (invalidated)} & \textit{---} & \textit{0.3920 (39.2\%) leakage} \\
\bottomrule
\end{tabular}
\end{table}

\noindent \textbf{Key finding}: After removing the leaky surprise modality,
GAT weight increases from 18.9\% to 41.2\%, becoming the dominant modality.
This confirms that V1's surprise dominance (39.2\%) was entirely driven by leakage---
the model had learned to read the answer from the input.

\subsection{V2 vs V1 Comparison}

\begin{table}[H]
\centering
\caption{V1 vs V2 Fusion Model Comparison}
\begin{tabular}{lrrl}
\toprule
\textbf{Metric} & \textbf{V1} & \textbf{V2} & \textbf{Change} \\
\midrule
Direction AUC   & 0.5161 (5d)  & \textbf{0.5989} (60d) & $+0.083$ (+16.1\%) \\
Volatility R$^2$ & 0.4115       & 0.3350                & $-0.077$ \\
Modalities      & 5 (incl. leaky) & 4 (clean)          & fewer, cleaner \\
Primary horizon & 5-day        & 60-day                & longer \\
Loss            & CE+MSE+CE    & 0.7$\times$ListNet + 0.3$\times$MSE & ranking \\
Batch size      & 32           & 1024                  & 32$\times$ larger \\
AMP             & No           & Yes (fp16)            & enabled \\
GPU: VRAM used  & $\sim$0.3 GB & $\sim$0.68 GB         & 2.3$\times$ more \\
\bottomrule
\end{tabular}
\end{table}

\noindent The 60-day horizon is the primary driver of AUC improvement (Phase 8
demonstrated 60-day AUC of 0.569 vs 5-day 0.524 on the V1 baseline). The ListNet
ranking loss contributes incremental improvement over the 60-day CE baseline.

\subsection{V2 Ablation: V1 vs V2 AUC Progression}

\begin{table}[H]
\centering
\caption{Cross-Pipeline AUC Progression (direction prediction)}
\begin{tabular}{llr}
\toprule
\textbf{Phase} & \textbf{Model} & \textbf{AUC} \\
\midrule
V1 Phase 4 & Price (5d)                  & 0.5241 \\
V1 Phase 5 & GAT (5d)                    & 0.5257 \\
V1 Phase 6 & Fusion 5-modality (5d)      & 0.5161 \\
V1 Phase 8 & V2 ranking loss (5d)        & 0.5249 \\
V1 Phase 8 & V2 ranking loss (60d)       & 0.5312 \\
V1 Phase 9C & Best clean model           & 0.5312 \\
\midrule
V2 Phase 4A & Price CNN-BiLSTM (60d)     & 0.5504 \\
V2 Phase 5  & GAT static (60d)           & 0.5777 \\
\textbf{V2 Phase 6} & \textbf{Fusion 4-mod, ListNet (60d)} & \textbf{0.5989} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{V2 Summary}

The V2 pipeline achieves a direction AUC of \textbf{0.5989} on the 60-day horizon
with 4 clean modalities and ListNet ranking loss. Compared to V1's best valid result
(AUC=0.5312), this represents a $+0.069$ ($+13\%$) improvement.

Key lessons from V1 $\to$ V2 transition:
\begin{enumerate}
    \item \textbf{Data leakage is the primary threat} to validity: surprise features inflated V1
          AUC by masking the actual signal strength.
    \item \textbf{Longer horizons are easier to predict}: 60-day direction is more predictable
          than 5-day, as short-term noise dominates the latter.
    \item \textbf{GAT is the highest-value modality} once leakage is removed, contributing
          +2.6\% AUC in isolation and holding 41.2\% gate weight in the clean fusion.
    \item \textbf{Macro features (new)} contribute a modest 12.6\% gate weight,
          suggesting market regime information is partially captured but not dominant.
    \item \textbf{ListNet ranking loss} improves cross-stock relative ranking by removing
          common market beta, but AUC improvement over 60d CE is modest ($\sim$+0.015).
\end{enumerate}

%% ====================================================================
\section{Phase 11: Targeted Improvements}
%% ====================================================================

Phase 11 implemented three targeted improvements to the V2 pipeline:
(A) Confidence filtering on V2 and V2.1 fusion outputs,
(B) Expanded macro encoder from 32-d to 128-d (V2.1), and
(C) a weekly long-short backtesting framework.
All results are reported on the held-out test set (2024--2025).

%% ------------------------------------------------------------------
\subsection{Step 1: GPU Utilization Audit}
%% ------------------------------------------------------------------

A GPU diagnostic was run on the RTX 2050 (4.29~GB VRAM, compute 8.6).
Key findings and fixes applied:

\begin{itemize}
    \item \textbf{cuDNN benchmark}: was OFF at startup; now enabled via
          \texttt{setup\_gpu()}.
    \item \textbf{TF32 matmul/cuDNN}: both now enabled.
    \item \textbf{DataLoader workers}: \texttt{num\_workers=0} is faster on
          Windows due to Python process-spawn overhead; applied in
          \texttt{make\_dataloader()} with platform detection.
    \item \textbf{GPU preloading}: \texttt{FusionEmbeddingDataset.to\_device()}
          pre-loads all tensors to GPU at dataset creation, eliminating
          per-batch host$\to$device copies.
    \item \textbf{Batch size}: increased from 1024 to 4096 for better GPU
          utilisation.
    \item \textbf{Honest assessment}: peak VRAM is only $\sim$1.0~GB even at
          batch~4096; the $\sim$563K-parameter fusion model is too small to
          saturate the 4~GB VRAM on pre-extracted embeddings.
\end{itemize}

%% ------------------------------------------------------------------
\subsection{Step 2: Dead Code Cleanup}
%% ------------------------------------------------------------------

Removed orphaned source files and large checkpoints freed from the repo:

\begin{itemize}
    \item Deleted source files: \texttt{news\_model.py}, \texttt{news\_dataset.py},
          \texttt{train\_news.py}, \texttt{dynamic\_graph.py}
    \item Deleted model checkpoints: \texttt{news\_model\_best.pt} ($\sim$450~MB),
          \texttt{dynamic\_gat\_best.pt} ($\sim$9.7~MB),
          \texttt{phase9\_regime/} directory ($\sim$6.5~MB)
    \item Total freed: $\sim$466~MB
    \item Retained: \texttt{graph\_nogat\_best.pt} (ablation reference)
\end{itemize}

%% ------------------------------------------------------------------
\subsection{Phase 11B: Macro Encoder Expansion (32-d $\to$ 128-d)}
%% ------------------------------------------------------------------

\subsubsection{Motivation}
The V2 macro encoder used a small 3-layer MLP
(12$\to$64$\to$64$\to$32) and held only 12.6\% gate weight in the
fusion model. Expanding the encoder was hypothesised to extract richer
macro state representations.

\subsubsection{Architecture Change}
The macro encoder was widened to
12$\to$128$\to$128$\to$128 (hidden\_dim=128, output\_dim=128, dropout=0.1).
All other modalities (price 256-d, GAT 256-d, doc 768-d) and the fusion
head were kept identical.

\subsubsection{Results (V2 vs V2.1)}

\begin{center}
\begin{tabular}{lccr}
\hline
\textbf{Metric}   & \textbf{V2 (32-d)} & \textbf{V2.1 (128-d)} & \textbf{$\Delta$} \\
\hline
Direction AUC     & 0.5989             & 0.5917                & $-0.0072$ \\
Direction Acc     & 0.5920             & 0.5544                & $-0.0376$ \\
Direction F1      & N/A                & 0.5944                & --- \\
Vol R$^2$         & 0.3350             & 0.1154                & $-0.2196$ \\
\hline
Gate: Price       & 23.6\%             & 12.4\%                & $-11.2$pp \\
Gate: GAT         & 41.2\%             & 38.1\%                & $-3.1$pp \\
Gate: Doc         & 22.7\%             & 17.8\%                & $-4.9$pp \\
Gate: Macro       & 12.6\%             & 31.7\%                & $+19.1$pp \\
\hline
\end{tabular}
\end{center}

\subsubsection{Analysis}
The expanded macro encoder significantly increased macro gate weight
(12.6\% $\to$ 31.7\%, a $2.5\times$ increase), confirming that the
richer embedding captures more information the fusion head finds useful.
However, both directional AUC and volatility R$^2$ decreased.
Two likely explanations:
\begin{enumerate}
    \item The 128-d macro encoder requires stronger regularisation to
          avoid overfitting on the small macro dataset.
    \item The 128-d macro embedding dominates the fusion too aggressively,
          reducing the contribution of the well-trained GAT and price
          modalities.
\end{enumerate}
\textbf{Verdict}: V2 (32-d macro) remains the best overall model by AUC.

%% ------------------------------------------------------------------
\subsection{Phase 11A: Confidence Filtering}
%% ------------------------------------------------------------------

\subsubsection{Method}
For each test sample, the maximum predicted probability across the two
direction classes is computed: $c = \max(P(\text{UP}), P(\text{DOWN}))$.
Samples with $c < \theta$ are excluded. Thresholds
$\theta \in \{0.50, 0.52, 0.54, \ldots, 0.70\}$ were swept.

\subsubsection{V2 Confidence Filtering}
\textbf{Conclusion}: Filtering degrades V2 performance.
The best AUC (0.6149) occurs at full coverage ($\theta=0.50$).
Higher thresholds progressively reduce AUC and F1 collapses to 0.0
at $\theta \geq 0.55$ (model degenerates to all-one-class predictions).
V2 has poor output calibration --- insufficient confidence spread.

\subsubsection{V2.1 Confidence Filtering Results}
\begin{center}
\begin{tabular}{rrrrrr}
\hline
\textbf{Thresh} & \textbf{N} & \textbf{Cov.} & \textbf{AUC} & \textbf{Acc} & \textbf{F1} \\
\hline
0.50 & 9520 & 100.0\% & 0.5918 & 0.5544 & 0.5944 \\
0.52 & 8216 &  86.3\% & 0.6001 & 0.5565 & 0.5909 \\
0.54 & 6934 &  72.8\% & 0.6068 & 0.5509 & 0.5645 \\
0.56 & 5588 &  58.7\% & 0.6126 & 0.5311 & 0.4938 \\
\textbf{0.60} & \textbf{3068} & \textbf{32.2\%} & \textbf{0.6127} & 0.4922 & 0.2010 \\
0.62 & 2164 &  22.7\% & 0.5765 & 0.5185 & 0.0844 \\
\hline
\end{tabular}
\end{center}

\textbf{Conclusion}: V2.1 is better calibrated than V2.
Filtering at $\theta=0.60$ achieves AUC~0.6127 (+0.021 over V2.1 full,
at 32.2\% coverage). This represents a precision--recall trade-off:
higher-confidence predictions are more accurate but only 1-in-3 samples
qualify. F1 degrades significantly at high thresholds as the model
becomes conservative.

%% ------------------------------------------------------------------
\subsection{Phase 11C: Weekly Long-Short Backtest}
%% ------------------------------------------------------------------

\subsubsection{Setup}
\begin{itemize}
    \item \textbf{Model}: V2 fusion (macro 32-d)
    \item \textbf{Universe}: 10 tickers (S\&P 500 constituents)
    \item \textbf{Portfolio}: Top-2 long / Bottom-2 short by predicted UP probability
    \item \textbf{Rebalancing}: Every 5 trading days (weekly)
    \item \textbf{Position sizing}: Equal weight and confidence-weighted variants
    \item \textbf{Test period}: 384 trading days (96 weekly rebalances), 2024--2025
    \item \textbf{Transaction costs}: 0, 5, 10, 20 basis points (one-way)
\end{itemize}
...
\hline
\textbf{Sizing}   & \textbf{Cost} & \textbf{Total Ret.} & \textbf{Sharpe} & \textbf{Max DD} & \textbf{Win Rate} \\
\hline
Equal weight      & 0 bp   & $+10.4$\%  & 0.180  & $-19.0$\% & 51.9\% \\
Equal weight      & 5 bp   & $+8.7$\%   & 0.123  & $-19.4$\% & 50.6\% \\
Equal weight      & 10 bp  & $+7.0$\%   & 0.067  & $-19.8$\% & 50.6\% \\
Equal weight      & 20 bp  & $+3.7$\%   & $-0.046$ & $-20.7$\% & 49.4\% \\
Conf-weighted     & 0 bp   & $+6.7$\%   & 0.063  & $-18.9$\% & 51.9\% \\
Conf-weighted     & 5 bp   & $+5.1$\%   & 0.009  & $-19.3$\% & 51.9\% \\
Conf-weighted     & 10 bp  & $+3.4$\%   & $-0.045$ & $-19.7$\% & 51.9\% \\
Conf-weighted     & 20 bp  & $+0.3$\%   & $-0.154$ & $-20.6$\% & 49.4\% \\
\hline
\end{tabular}
\end{center}

Break-even costs: equal weight $\approx$10~bp; confidence-weighted $\approx$5~bp.
Average weekly turnover: 20.3\%.

\subsubsection{Analysis}
\begin{itemize}
    \item The equal-weight strategy generates positive returns at realistic
          cost levels (10~bp break-even covers most institutional trading costs).
    \item Confidence-weighted sizing slightly underperforms equal weight,
          consistent with V2's poor output calibration.
    \item Sharpe ratios are modest (0.067--0.180); the strategy wins more
          often than not (51.9\%) but with significant drawdown.
    \item Maximum drawdown of 19--21\% is large, driven by the small
          10-ticker universe.
\end{itemize}

%% ------------------------------------------------------------------
\subsection{Phase 11 Summary}
%% ------------------------------------------------------------------

\begin{center}
\begin{tabular}{lcc}
\hline
\textbf{Model / Configuration}            & \textbf{AUC}  & \textbf{Notes} \\
\hline
V2  (full, macro 32-d)                    & 0.5989        & Phase 10 baseline \\
V2.1 (full, macro 128-d)                  & 0.5917        & $-0.0072$ vs V2 \\
V2 + confidence filter ($\theta=0.50$)    & 0.6149        & No filtering (full = best) \\
V2.1 + confidence filter ($\theta=0.60$)  & 0.6127        & 32.2\% coverage \\
\hline
\end{tabular}
\end{center}
...
Key findings:
\begin{enumerate}
    \item \textbf{Macro expansion} increases macro gate weight by $2.5\times$
          but does not improve directional accuracy. V2 (32-d macro)
          remains the best overall fusion model.
    \item \textbf{Confidence filtering} is only beneficial on V2.1:
          +0.021 AUC at 32\% coverage. V2 has poor output calibration
          and gains nothing from filtering.
    \item \textbf{Weekly backtest} shows the V2 signal is tradeable at
          $\leq$10~bp transaction costs; Sharpe 0.18 at zero cost
          and 0.07 at 10~bp.
    \item The best overall AUC across all Phase~11 configurations
          is \textbf{0.6149} (V2, full coverage).
\end{enumerate}
...
%% ==================================================================
\newpage
\section{Phase 12: Volatility-Primary Redesign with Universe Expansion}

\subsection{Motivation}
Phase~12 makes two simultaneous changes to the V2 pipeline:

\begin{enumerate}
    \item \textbf{Primary target switch:} Direction $\to$ Volatility.
    Previous loss: $0.7\times\text{ListNet}(\text{dir}_{60d}) + 0.3\times\text{MSE}(\text{vol})$.
    New loss: $0.85\times\text{QLIKE}(\text{vol}) + 0.15\times\text{ListNet}(\text{dir}_{60d})$.
    Rationale: volatility is persistent (GARCH clustering) while
    direction is near-random (EMH). Optimizing primarily for volatility
    should yield substantially higher $R^2$.

    \item \textbf{Universe expansion:} 10 $\to$ 30 stocks.
    Added 20 stocks across 5 sectors: Semiconductors (QCOM, TXN, AVGO, MRVL, KLAC),
    ...
    ListNet ranking with 30 stocks per date is $3\times$ richer than with 10.
\end{enumerate}

\subsection{Architecture Changes}

\subsubsection{Phase12FusionModel}
Key differences from V2 \texttt{MultimodalFusionModel}:
\begin{itemize}
    \item \textbf{Volatility head:} Deeper MLP with \texttt{Softplus} activation
          guaranteeing positive output ($\hat{\sigma} > 0$).
    \item \textbf{Direction head:} Lightweight auxiliary (fewer parameters).
    \item \textbf{MC Dropout:} Dropout remains active at inference for
          epistemic uncertainty estimation ($n=30$ forward passes).
    \item Total parameters: 559,047 (V2: 545,923).
\end{itemize}

\subsubsection{QLIKE Loss}
The Quasi-Likelihood loss (Patton, 2011):
\[
    \mathcal{L}_{\text{QLIKE}} = \frac{1}{N}\sum_{i=1}^N
    \left(\frac{\sigma^2_{\text{true},i}}{\hat{\sigma}^2_i}
    + \log\hat{\sigma}^2_i\right)
\]
Scale-sensitive; penalizes under-prediction of variance more heavily.

\subsubsection{Feature Engineering}
Eight new volatility-specific features (total: 18):
\begin{enumerate}
    \item Parkinson volatility (high/low range, 10-day rolling)
    \item Garman-Klass volatility (OHLC, 10-day rolling)
    \item 5-day realized volatility
    \item Volatility ratio (short/long regime indicator)
    \item Squared return (instantaneous variance proxy)
    \item Absolute return (robust volatility proxy)
    \item Jump indicator ($|r_t| > 3\sigma_{20d}$)
    \item EWMA variance (exponentially weighted, span=10)
\end{enumerate}

\subsubsection{Graph Expansion}
30-node knowledge graph with 70 directed edges (170 bidirectional + 30 self-loops = 200 total).
Relation types: supplier\_to, platform\_competitor, ad\_competitor, cloud\_competitor,
consumer\_attention\_overlap, enterprise\_competitor, index\_correlation.

\subsection{Training Configuration}
\begin{itemize}
    \item Component models: Price (CNN-BiLSTM, 18 features), GAT (30 nodes, 18 features)
    \item Document model: FinBERT reused from V2 (227 filings for 30 stocks)
    \item Macro model: reused from V2 (12 macro features $\to$ 32-d embedding)
    \item Fusion: AdamW lr=$5\times10^{-4}$, CosineAnnealing, 50 epochs, batch 2048
    \item AMP (fp16) throughout; RTX 2050 4.3~GB VRAM
\end{itemize}
...
\begin{tabular}{lrrr}
\hline
\textbf{Split} & \textbf{Samples} & \textbf{Mean Vol} & \textbf{Dir \%UP} \\
\hline
Train ($\leq$ 2022-12-31) & 48,532 & 0.356 & 63.2\% \\
Val (2023)                & 8,057  & 0.348 & 73.7\% \\
Test ($>$ 2023-12-31)     & 17,204 & 0.396 & 58.8\% \\
\hline
\end{tabular}
\end{center}
...
\begin{tabular}{lccccc}
\hline
\textbf{Model} & \textbf{Vol $R^2$} & \textbf{Vol MAE} & \textbf{Vol RMSE}
               & \textbf{Dir AUC} & \textbf{Dir Acc} \\
\hline
V2 Baseline         & 0.335  & ---    & ---    & 0.5989 & ---    \\
Historical Average  & 0.348  & 0.112  & 0.157  & ---    & ---    \\
HAR-RV              & 0.947  & 0.018  & 0.045  & ---    & ---    \\
\textbf{Phase 12}   & \textbf{0.772} & \textbf{0.062} & \textbf{0.096}
                    & 0.568  & 0.570  \\
\hline
\end{tabular}
\end{center}

\subsection{Embedding Coverage}

\begin{center}
\begin{tabular}{lr}
\hline
\textbf{Modality} & \textbf{Coverage} \\
\hline
Price (CNN-BiLSTM)    & 73,793 / 73,793 (100.0\%) \\
GAT (30-node graph)   & 73,292 / 73,793 (99.3\%) \\
Document (FinBERT)    & 54,362 / 73,793 (73.7\%) \\
Macro (MLP-3L)        & 73,468 / 73,793 (99.6\%) \\
\hline
\end{tabular}
\end{center}
...
\subsection{Key Findings}
\begin{enumerate}
    \item \textbf{Volatility $R^2$ improved from 0.335 to 0.772} ($+130\%$),
          confirming that QLIKE-primary optimization with volatility-specific
          features dramatically outperforms MSE-auxiliary volatility prediction.
    \item \textbf{Direction AUC decreased from 0.599 to 0.568} ($-5\%$),
          as expected when direction becomes a $15\%$-weighted auxiliary task.
          The trade-off is justified: volatility is the actionable signal.
    \item \textbf{HAR-RV remains a strong univariate baseline} ($R^2=0.947$)
          because it directly auto-regresses on lagged realized volatility.
          The multimodal model uses fundamentally different inputs (embeddings
          from price patterns, graph attention, documents, macro) and achieves
          $R^2=0.772$ without explicitly encoding lagged RV.
    \item \textbf{Universe expansion to 30 stocks} increased training data
          by $\sim3\times$ and enriched ListNet ranking diversity.
          Document coverage expanded from $\sim52$ to 227 filings.
    \item \textbf{Softplus volatility head} guarantees positive predictions,
          eliminating the need for post-hoc clipping that MSE-based models require.
    \item \textbf{MC Dropout uncertainty} enables confidence-weighted trading
          strategies in downstream applications.
\end{enumerate}

%% ============================================================
%% PHASE 13: HAR-RV Integration + Loss Optimization
%% ============================================================
\section{Phase 13: HAR-RV Integration, Loss Optimization, and Volatility Trading Strategy}

\subsection{Motivation}
Phase~12 achieved volatility $R^2=0.772$ without explicitly encoding lagged realized
volatility, while the pure HAR-RV baseline reached $R^2=0.947$.  Phase~13 closes this
gap by (1)~adding HAR-RV autoregressive features directly into the price feature set,
(2)~systematically varying the QLIKE/ListNet loss weighting, and (3)~evaluating whether
improved volatility predictions translate to profitable trading strategies.

\subsection{HAR-RV Feature Engineering}
Three autoregressive features were added to the price feature pipeline, expanding the
engineered feature count from 18 to 21:
\begin{enumerate}
    \item \textbf{rv\_lag1d}: Daily lagged realized volatility---annualized standard
          deviation of log returns over a 20-day rolling window, shifted by 1 day.
    \item \textbf{rv\_lag5d}: Weekly lagged RV---5-day rolling mean of daily RV,
          shifted by 1 day (Corsi 2009 weekly component).
    \item \textbf{rv\_lag22d}: Monthly lagged RV---22-day rolling mean of daily RV,
          shifted by 1 day (Corsi 2009 monthly component).
\end{enumerate}
All three features use \texttt{shift(1)} to prevent look-ahead bias.  Leakage
validation confirmed: correlation of rv\_lag1d with the forward volatility target
is 0.454, while correlation with the direction label is 0.017 (consistent with
information content rather than data leakage).

\subsection{Loss Weight Variants}
Three fusion model variants were trained with identical architecture
(Phase13FusionModel, 559,047 parameters) but different QLIKE/ListNet loss weights:
\begin{center}
\begin{tabular}{lccccc}
\toprule
\textbf{Variant} & $\lambda_{\text{vol}}$ & $\lambda_{\text{dir}}$ &
\textbf{Vol $R^2$} & \textbf{RMSE} & \textbf{Dir AUC} \\
\midrule
vol\_primary   & 0.85 & 0.15 & \textbf{0.8665} & \textbf{0.0733} & 0.5848 \\
balanced       & 0.50 & 0.50 & 0.7368 & 0.1029 & 0.5788 \\
dir\_assisted  & 0.70 & 0.30 & 0.8502 & 0.0777 & \textbf{0.5854} \\
\bottomrule
\end{tabular}
\end{center}
The vol\_primary variant (0.85/0.15) achieves the best volatility $R^2$,
while dir\_assisted (0.70/0.30) offers the best direction AUC with only a modest
$R^2$ decrease.  The balanced variant (0.50/0.50) underperforms on \emph{both}
objectives, confirming that competing QLIKE and ListNet gradients at equal weight
produce unstable optimization (early-stopped at epoch~10 vs 40 and 26 for the others).
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Vol $R^2$} & \textbf{RMSE} & \textbf{Dir AUC} & \textbf{$\Delta$ vs P12} \\
\midrule
Historical Average & 0.348 & --- & --- & --- \\
V2 Baseline        & 0.335 & --- & --- & --- \\
HAR-RV             & \textbf{0.947} & --- & --- & --- \\
Phase 12 Fusion    & 0.772 & 0.096 & 0.568 & (ref) \\
\textbf{Phase 13 vol\_primary} & \underline{0.867} & \textbf{0.073} & 0.585 & +12.3\% \\
Phase 13 balanced  & 0.737 & 0.103 & 0.579 & $-4.5\%$ \\
Phase 13 dir\_assisted & 0.850 & 0.078 & \textbf{0.585} & +10.1\% \\
\bottomrule
\end{tabular}
\end{center}

Phase~13 does \emph{not} beat HAR-RV ($R^2=0.947$), but the gap is reduced from
0.175 (Phase~12) to 0.080 (Phase~13 vol\_primary)---a 54\% reduction.  The remaining
gap reflects that HAR-RV directly auto-regresses on the target variable, while the
multimodal model must learn this relationship indirectly through embeddings.

\subsection{Gate Weight Analysis}
The vol\_primary attention gates assign: Price=61.8\%, GAT=4.9\%, Document=17.8\%,
Macro=15.6\%.  Price dominance is expected given the HAR-RV autoregressive features
now encoded in the price embedding.  Document weight (17.8\%) confirms that SEC
filings provide complementary forward-looking volatility information beyond what
pure price history captures.

\subsection{Volatility Trading Strategy}
A market-neutral long/short strategy was constructed:
\begin{itemize}
    \item \textbf{Signal}: ratio of predicted volatility to historical average
          volatility per stock.
    \item \textbf{Position sizing}: uncertainty-weighted (MC Dropout with 30 forward
          passes), weekly rebalancing.
    \item \textbf{Long}: stocks with high predicted/historical vol ratio;
          \textbf{Short}: low ratio.
\end{itemize}

\begin{center}
\begin{tabular}{lccc}
\toprule
\textbf{Strategy} & \textbf{Sharpe} & \textbf{Total Return} & \textbf{Max DD} \\
\midrule
Vol Strategy (0bp)  & $-0.453$ & $-1.83\%$ & --- \\
Dir Strategy (0bp)  & 0.604   & 11.48\%   & --- \\
Buy \& Hold (EW 30) & 0.652   & 22.28\%   & --- \\
\bottomrule
\end{tabular}
\end{center}

The volatility strategy underperforms because (1)~predicting future volatility
\emph{levels} well does not imply profitable volatility \emph{mispricing} signals,
and (2)~the test period (2024--2025) is a strong bull market where directional
exposure dominates market-neutral strategies.  The direction strategy achieves a
positive Sharpe (0.604) despite modest AUC (0.585), confirming that even weak
directional signals compound profitably with sufficient diversification across
30~stocks.

\subsection{Phase 13 Key Takeaways}
\begin{enumerate}
    \item \textbf{HAR-RV features close 54\% of the gap to the HAR-RV benchmark},
          improving vol $R^2$ from 0.772 to 0.867 ($+12.3\%$).
    \item \textbf{Loss weight 0.85/0.15 is optimal for volatility}, confirming
          that heavy QLIKE weighting is necessary for accurate vol forecasting.
    \item \textbf{Direction AUC improves despite lower weight} (0.568 $\to$ 0.585),
          because HAR-RV features provide auxiliary directional information.
    \item \textbf{Volatility prediction quality does not directly translate to
          profitable equity strategies}---options or variance swap strategies
          would be more natural applications.
    \item \textbf{Balanced (0.50/0.50) weighting is harmful}---competing objectives
          at equal strength produce unstable training and inferior results on
          \emph{both} tasks.
\end{enumerate}

%% ====================================================================
\section{Phase 14: HAR-RV Skip Connection and Fixed Volatility Strategy}
%% ====================================================================

\subsection{Motivation}

Phase~13 achieved vol~$R^2 = 0.867$, still $0.080$ below HAR-RV ($R^2 = 0.947$).
The gap exists because the CNN-BiLSTM compresses the $[60, 21]$ price sequence
into a 256-dimensional embedding, losing some of the autoregressive signal that
HAR-RV exploits directly via lagged realized volatility.  Two targeted
improvements were implemented: (1)~a skip connection feeding raw HAR-RV features
directly into the fusion trunk, bypassing the CNN-BiLSTM bottleneck; (2)~a
VIX~$\times$~$\beta$ implied volatility proxy to transform the raw vol strategy
into a mispricing signal.

\subsection{HAR-RV Skip Connection Architecture}

The only structural change from Phase~13 is the addition of a 3-dimensional skip
path.  From the last timestep of the $[60, 21]$ price tensor, features at
indices 18--20 (\texttt{rv\_lag1d}, \texttt{rv\_lag5d}, \texttt{rv\_lag22d}) are
extracted and projected through a small network ($3 \to 32$ with LayerNorm and
GELU).  The 32-dimensional projection is concatenated \emph{directly} to the
gated modality features before the shared trunk, bypassing the gating mechanism
entirely:
\[
  \mathbf{z}_{\text{trunk}} = [\mathbf{p}_g,\, \mathbf{g}_g,\, \mathbf{d}_g,\,
  \mathbf{m}_g,\, \text{HAR-proj}(\mathbf{rv}_{-1})]
  \in \mathbb{R}^{4 \times 128 + 32 = 544}
\]
where subscript $g$ denotes gated projections.  Total parameters increased from
559,303 (Phase~13) to 567,431 (Phase~14, $+1.5\%$).

\subsection{Volatility Forecasting Results}

Training used 0.85~QLIKE + 0.15~ListNet, AdamW ($\text{lr}=5\times10^{-4}$,
$\text{wd}=10^{-3}$), cosine annealing over 60 epochs with patience-7 early
stopping.  The model converged at epoch~26.

\begin{center}
\begin{tabular}{l r r r r}
\hline
\textbf{Model} & \textbf{Vol $R^2$} & \textbf{RMSE} & \textbf{QLIKE} & \textbf{Dir AUC} \\
\hline
Historical Average & 0.348 & --- & --- & --- \\
V2 Baseline        & 0.335 & --- & --- & --- \\
HAR-RV             & 0.947 & 0.045 & --- & --- \\
Phase~12           & 0.772 & 0.096 & --- & 0.568 \\
Phase~13           & 0.867 & 0.073 & --- & 0.585 \\
Phase~14 (skip)    & 0.921 & 0.056 & $-0.994$ & 0.591 \\
\hline
\end{tabular}
\end{center}

Phase~14 \textbf{does not beat HAR-RV} ($R^2 = 0.921$ vs.\ 0.947, gap~$= 0.026$).
However, it closed $0.586$ of the $0.612$ total gap between V2 Baseline and
HAR-RV (95.7\%).  The remaining 0.026 likely reflects the irreducible advantage
of a pure linear model on strongly autoregressive data: HAR-RV's three
coefficients are estimated by OLS with zero compression, whereas the skip
connection still passes through a projection and nonlinearity.

Gate weights shifted toward price (58.6\%) as the skip connection captured the
autoregressive signal, reducing reliance on document (19.5\%) and macro (6.8\%).
The HAR-RV skip contribution (mean absolute difference between predictions with
and without the skip) was 0.115, confirming active utilization.

\subsection{VIX-Adjusted Volatility Trading Strategy}

Phase~13's raw vol strategy failed (Sharpe~$= -0.453$) because it went long
high-predicted-vol stocks without reference to what the market already priced in.
Phase~14 used VIX~$\times$~$\beta_i$ as a proxy for stock~$i$'s market-implied
volatility, where $\beta_i$ was estimated from trailing 252-day returns against
SPY.  The signal became $\sigma_{\text{pred}} - \sigma_{\text{implied}}$
(positive~$\to$ underpriced, short when overpriced).

\begin{center}
\begin{tabular}{l r r r}
\hline
\textbf{Strategy} & \textbf{Sharpe} & \textbf{Ann.\ Return} & \textbf{Max DD} \\
\hline
Vol raw (0bp)       & $-0.763$ & $-6.4\%$ & $-26.3\%$ \\
Vol VIX-adj (0bp)   & $-1.178$ & $-9.3\%$ & $-20.0\%$ \\
Vol VIX-adj (5bp)   & $-1.241$ & $-10.1\%$ & $-21.1\%$ \\
Vol VIX-adj (10bp)  & $-1.304$ & $-10.8\%$ & $-22.1\%$ \\
\hline
\end{tabular}
\end{center}

The VIX-adjusted strategy did \textbf{not} improve over the raw vol strategy.
VIX~$\times$~$\beta$ is a market-level proxy; individual stock implied
volatility from options data would be needed for meaningful mispricing detection.

\subsection{Direction Strategy Cost Sensitivity}

\begin{center}
\begin{tabular}{l r r r}
\hline
\textbf{Cost} & \textbf{Sharpe} & \textbf{Ann.\ Return} & \textbf{Max DD} \\
\hline
0bp  & $+0.697$ & $+12.9\%$ & $-11.4\%$ \\
5bp  & $+0.620$ & $+12.0\%$ & $-11.6\%$ \\
10bp & $+0.543$ & $+11.2\%$ & $-11.9\%$ \\
20bp & $+0.392$ & $+9.5\%$  & $-12.4\%$ \\
\hline
\end{tabular}
\end{center}

The direction strategy remains viable at realistic transaction costs (Sharpe
$> 0$ at 20bp), making it the most practical trading application of the model.

\subsection{Phase 14 Key Findings}

\begin{enumerate}
    \item \textbf{HAR-RV skip connection improved vol $R^2$ from 0.867 to
          0.921} ($+6.3\%$), closing 95.7\% of the gap from V2 to HAR-RV.
    \item \textbf{The skip connection is actively used} (contribution metric
          $= 0.115$), confirming the CNN-BiLSTM bottleneck was a real
          information loss.
    \item \textbf{Phase~14 does not beat HAR-RV} ($R^2 = 0.921$ vs.\ 0.947).
          The remaining gap of 0.026 reflects HAR-RV's zero-compression
          advantage on strongly autoregressive data.
    \item \textbf{VIX~$\times$~$\beta$ is too crude} for individual stock
          implied volatility; the mispricing strategy requires actual options
          data.
    \item \textbf{The direction strategy survives realistic costs}: Sharpe
          $= 0.620$ at 5bp, $0.543$ at 10bp, $0.392$ at 20bp.
    \item \textbf{Dir AUC improved slightly} from 0.585 (Phase~13) to 0.591
          (Phase~14), suggesting the skip connection provides marginal
          directional information.
\end{enumerate}

\subsection{Complete Project Summary}
...
Phase 14 & Skip Connection         & 0.921 & 0.591 \\
HAR-RV   & Benchmark               & 0.947 & --- \\
\hline
\end{tabular}
\end{center}

Over Phases~12--14, volatility $R^2$ improved from 0.335 to 0.921—a 2.75$\times$
improvement driven by QLIKE loss, HAR-RV autoregressive features, and the skip
connection architecture.  The direction strategy produces positive risk-adjusted
returns at realistic costs, while the volatility model approaches—but does not
surpass—the HAR-RV econometric benchmark.
